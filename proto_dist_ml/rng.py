...
class RNG(BaseEstimator, ClusterMixin):
    ...
    lambda_0: float (optional, default = K / 2)
        The initial neighborhood range, i.e. the rank on which the prototype
        influence on a datapoint degrades to 1/e.
    _Alpha: array_like
        A K x m matrix sparse matrix storing the convex coefficients that
    ...
    _loss: array_like
        The relational neural gas loss during the last training run.
        This is not set by the user but during fit().
    ...
    def fit(self, D, y = None, is_squared = False):
        ...
        .. math:: d(w_k, x_i)^2 = A_{k, :} \\cdot D^2_{:, i} + z_k

        2. We compute the ranks of each prototype for each data point, i.e.

        .. math:: r_i(k) = |\\{ l | d(w_l, x_i)^2 < d(w_k, x_i)^2 \\}|

        3. We update the coefficients _Alpha via the formula

        .. math:: A_{k, i} = \\frac{\\exp(-r_i(k) / \\lambda)}{\\sum_l \\exp(-r_i(l) / \\lambda)}
        ...
        ----------
        D: array_like
            A m x m matrix of pairwise distances, which need to be symmetric
            and zero on the diagonal.
        y: array_like
            This parameter is ignored and is only here for API consistency.
    ...
    def predict(self, D, is_squared = False):
        ...
        -------
        y: array_like
           A n-dimensional vector containing the cluster indices for each
           datapoint.
